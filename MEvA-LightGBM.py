...
class Individual():
	...
	def evaluate(self):
		...
		if mask.any():
            num_rounds = 1000
            lgb_params = {
                'num_leaves': int(self.individual[4]), 
                'max_depth': int(self.individual[5]), 
                'learning_rate': self.individual[6],
                'n_estimators': int(self.individual[7]),
                'min_child_samples': int(self.individual[8]), 
                'subsample': self.individual[9],
                'colsample_bytree': self.individual[10],
                'n_jobs': -1,
                'metric': 'auc',
            }
            ...
            # Scaling the inputs with a robust scaler
            scaler = RobustScaler()
            ...
            for train_index, test_index in skf.split(inputs, outputs):
                ...
                predictions = clf.predict_proba(testing_inputs)[:, 1]+1e-08
